...
Mmax=1
Nmax=20000
coarseness=0.01 #acceptable fraction of leftover e-
...
def find_solution(NVal,T_e,fE,mur2,B,electrodeConfig,left,right,zpoints,rpoints,rfact,plotting):
    ...
    print(f'{(NVal+err):0.3e}\t{T_e:0.3e}\t{phi:0.3e}\t{n0:0.3e}\t{rmean:0.3e}\t{debye_length:0.3e}')
    ...
    return ngrid,position_map_z,position_map_r,voltageGuess,free_space_solution,rmean,voltageGuess
...
print('N \t T \t φ \t ρ \t r_0 \t λ_D')
fenow=1.8e5 #1.35
sol1=find_solution(NVal=110e6,T_e=50,fE=fenow,mur2=0.0015,B=2.0, #0.00145
                   electrodeConfig=(electrode_voltages,electrode_borders),
                   left=Llim,right=Rlim,zpoints=100,rpoints=20,rfact=3.0,plotting=True)
...
save=pd.DataFrame(n)
#save.to_csv('~/Documents/gnuplot/plasma2024/profiles/positron2D_hires',index=False,index_label=False,header=False)
#%%
save2=pd.DataFrame(sol1[6])
save3=pd.DataFrame(sol1[4])
save2.to_csv('~/Documents/gnuplot/plasma2024/pots/positron2Diii_pots',index=False,index_label=False,header=False)
#save3.to_csv('~/Documents/gnuplot/plasma2024/profiles/positron2D_freespace',index=False,index_label=False,header=False)
